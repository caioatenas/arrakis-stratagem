...
export function MovementAnimation({ originId, destinationId, quantity, playerColor, tipo = 'mover', onComplete }: MovementAnimationProps) {
  ...
  const isAttack = tipo === 'atacar';
  const trailColor = isAttack ? '#ef4444' : playerColor;
  const duration = 0.8;

  return (
    <g>
      {/* Glow trail */}
      <motion.path d={path} fill="none" stroke={trailColor} strokeWidth={isAttack ? 5 : 4} strokeLinecap="round"
      ...
      {Array.from({ length: particleCount }).map((_, i) => (
        <motion.circle key={i} r={isAttack ? 4.5 : 3.5} fill={trailColor} filter="url(#glow-selected)"
      ...
      <motion.g
        ...
        <circle cx={(origin.pos_x + dest.pos_x) / 2} cy={(origin.pos_y + dest.pos_y) / 2 - 5}
          r={12} fill="hsl(var(--background))" stroke={trailColor} strokeWidth={1.5} opacity={0.9} />
        <text x={(origin.pos_x + dest.pos_x) / 2} y={(origin.pos_y + dest.pos_y) / 2 - 2}
          textAnchor="middle" fill={trailColor} fontSize="9" fontFamily="Rajdhani, sans-serif" fontWeight="bold">
          {isAttack ? `⚔${quantity}` : quantity}
        </text>
      </motion.g>

      {/* Destination pulse / shake for attacks */}
      <motion.circle cx={dest.pos_x} cy={dest.pos_y} r={42}
        fill="none" stroke={trailColor} strokeWidth={isAttack ? 3 : 2}
        initial={{ scale: 0.8, opacity: 0 }}
        animate={{ scale: [0.8, 1.2], opacity: [0, 0.4, 0] }}
        transition={{ duration: 0.8, delay: 0.5 }}
        style={{ transformOrigin: `${dest.pos_x}px ${dest.pos_y}px` }} />

      {/* Attack: extra shake ring */}
      {isAttack && (
        <motion.circle cx={dest.pos_x} cy={dest.pos_y} r={36}
          fill="hsl(0, 70%, 50%)" fillOpacity={0}
          animate={{
            fillOpacity: [0, 0.15, 0],
            cx: [dest.pos_x, dest.pos_x + 3, dest.pos_x - 3, dest.pos_x + 2, dest.pos_x],
            cy: [dest.pos_y, dest.pos_y - 2, dest.pos_y + 3, dest.pos_y - 1, dest.pos_y],
          }}
